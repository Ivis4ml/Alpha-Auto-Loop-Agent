...
def run_research_loop(
    ...
    *,
    ...
    """执行一轮基线研究循环，返回摘要字典（同时写入 summary.json）。"""
    ...
    sink.install()
    try:
        return _run(store, config, generator, policy, out_dir, sealed_root)
    finally:
        sink.uninstall()
...
def _run(
    ...
    context = ResearchContext(
        field_scope=config.field_scope, root_seed=config.root_seed, feedback=None
    )
    specs = generator.propose(context, config.candidate_budget)
    ledger = TrialLedger(out_dir / "trials.jsonl")
    trial_numbers: dict[str, int] = {}
    for spec in specs:
        trial_numbers[spec.factor_id()] = ledger.register_planned(
            spec.factor_id(), {"generator": generator.generator_id, "spec": spec.tree}
        )
    emit("result", "候选登记", n_proposed=len(specs))
    ...
    emit("result", "筛查完成", n_accepted=len(screen.accepted), n_rejected=len(screen.rejected))
    ...
    emit("result", "样本内完成", n_survivors=len(survivors))
    ...
    fdr_pass = bh_fdr(p_values, q=config.gate.fdr_q)
    costs = CostModel(
    ...
    for record in candidates:
        ...
        if backtest["net_t"] > 0 and backtest["net_mean_daily"] > 0:
    ...
    summary = {
        ...
        "mode": config.mode,
        "protocol_id": config.protocol_id(),
        "n_proposed": len(specs),
        "n_rejected_static": len(screen.rejected),
